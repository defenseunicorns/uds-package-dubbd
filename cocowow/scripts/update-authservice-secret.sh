...
ENCODED_VALUE=$(kubectl get secret authservice -n authservice -o jsonpath='{.data.config\.json}'|base64 -d|sed "s/CHANGE_ME_MISSION_APP_SECRET/###ZARF_VAR_MISSION_APP_SECRET###/g"|base64|tr -d '\n' | sed 's/ //g')
